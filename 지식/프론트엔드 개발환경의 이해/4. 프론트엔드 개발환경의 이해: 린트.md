...
보프라기 있는 옷을 입을 수 있듯이 이러한 코드로 만드 어플리케이션도 동작은 한다.
...
브라우저는 코드에 세미콜론을 자동으로 넣는 과정(ASI)을 수행하는데, 위와 같은 경우는 우리의 의도대로 해석하지 못하고 아래 코드로 해석한다. ([Rules of Automatic Semicolon Insertion](http://www.ecma-international.org/ecma-262/7.0/index.html#sec-rules-of-automatic-semicolon-insertion) 을 참고)
...
ESLint는 ECMAScript 코드에서 문제점을 검사하는 일부는 더 나은 코드로 정정하는 린트 도구 중의 하나다.
...
이를 테면 "들여 쓰기 규칙", "코드 라인의 최대 너비 규칙"을 따르는 코드가 가독성이 더 좋다.
...
**"off"나 0은 끔, "wran"이나 1은 경고, "error"나 2는 오류**
...
이렇게 작성한 코드가 있다면 실수로 입력한 게 틀림 없다.
...
[규칙 목록](https://eslint.org/docs/rules/) 중 왼쪽에 렌치 표시(🔧)가 붙은 것이 `--fix` 옵션으로 자동 수정할 수 있는 규칙이다.

### 2.5 Extensible Config

이러한 규칙을 여러 개 미리 정해 놓은 것이 eslint:recommended 설정이다.

[규칙 목록](https://eslint.org/docs/rules/) 중에 왼쪽에 체크 표시(✓)되어 있는 것이 이 설정에서 활성화되어 있는 규칙이다.
...
```js
// app.js
console.log("hello world"); // 작은 따옴표, 뒤에 세미콜론 없는 경우
```
...
작은 따옴표를 큰 따옴표로 변경했다. 문자 뒤에 세미콜론도 추가했다.
...
ESLint는 [max-len](https://eslint.org/docs/rules/max-len) 규칙을 이용해 위 콛를 검사하고 결과만 알려 줄 뿐 수정하는 것은 개발자의 몫이다.
...
더 멋진 예제도 있는데 프리티어를 만든 [James Long의 글](https://archive.jlongster.com/A-Prettier-Formatter) 을 참고하자.

이러한 포맷팅 품질은 ESLint보다는 프리티어가 훨씬 좋은 결과를 만든다.
...
설정 파일에서 plugin와 rules에 설정을 추가한다.
...
이제 ESLint만 실행해도 프리티어 포맷팅 기능을 가져갈 수 있다.
...
커밋 전, 푸시 전 등 깃 커맨드 실행 시점에 끼여들 수 있는 훅을 제공한다.

[husky](https://github.com/typicode/husky) 는 깃 훅을 쉽게 사용할 수 있는 도구다.

(Git 2.13.0 이상 버전을 지원) 커밋 메세지 작성전에 끼어들어 린트로 코드 검사 작업을 추가하면 좋겠다.
...
커밋 메세지 작정 전에 lint-staged를 실행한 것이다.
...
![eslint 익스텐션 설치](https://jeonghwan-kim.github.io/static/a3c1eb59d1ab4df5020111f7c7347017/41099/eslint-extention.jpg)
...
![eslint 익스텐션 동작](https://jeonghwan-kim.github.io/static/85d33795b92ff1840fc8456ef184d2d4/b4294/eslint-extention-result.jpg)

툴팁 메뉴를 클릭해서 문제를 수정한다.

에디터 설정 중 저장시 액션을 추가할 수 있는데 ESLint로 코드를 정정할 수 있다.
...
이렇게 ESLint 익스텐션으로는 실시간 코드 품질 검사를 하고 저장시 자동 포맷팅을 하도록 하면 실시간으로 코드 품질을 검사하고 포맷도 일관적으로 유지할 수 있다.

## 5. 정리

읽기 좋은 코드는 유지보수 하기 좋다.

그만큼 어플리케이션의 수명은 오래갈 수 있다.

여럿이서 함께 일하는 환경에서 손으로 코드를 관리하는 것은 무척 번거롭고 어쩌면 불가능한 일일지도 모른다.

규칙이 정해졌고 자동화할 수 있다면 도구의 도움을 받는 것이 현명하다.

ESLint는 오류와 버그의 가능성을 찾아 코드 품질을 높이는 역할을 한다.

프리티어는 코드를 일관적으로 포맷팅하기 때문에 읽기 수월한 코드를 만들어 준다.

이러한 도구를 개발 플로우의 적절한 시점에 통합하여 자동화하면 개발자는 좀 더 본질적인 코딩에 집중할 수 있을 것이다.

출처 : [프론트엔드 개발환경의 이해: 린트](https://jeonghwan-kim.github.io/series/2019/12/30/frontend-dev-env-lint.html)
